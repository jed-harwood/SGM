...
if (is.null(script_path)) {
  script_path <- "GAR-Paper-Analyses/stock-auxiliary-scripts/stock_data_processing.R"
}
script_dir <- dirname(normalizePath(script_path, winslash = "/", mustWork = FALSE))
root_dir <- dirname(script_dir)

### Load the Data (taken from loggle package, used in Yang and Peng '19)
rm(list = setdiff(ls(), c("script_path", "script_dir", "root_dir")))
load(file.path(root_dir, "stockdata.rda"))
...
save.image(file = file.path(root_dir, "stock.data.X.Rdata"))
