...
* Step 1: given an initial estimate `$S$` for the covariance matrix (e.g., the sample covariance matrix) and an initial estimate for `$\theta_0$` (the reciprocal of the largest eigenvalue of `$S$`, squared.), use an ADMM algorithm to estimate the (normalized) graph Laplacian `$L$ `of the latent graph.  
...
* Step 3:
    ...
    * b. given `$v_0$` and the zero-pattern of `$L$` from Step 1, use an ADMM algorithm to simultaneously re-estimate `$theta_0$` and the (normalized) graph Laplacian `$L$`.
