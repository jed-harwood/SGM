...
* Step 3:
    * a. given the (normalized) graph Laplacian from Step 2, use an ADMM algorithm to estimate the normalized degree vector $v_0$.
    * b. given $v_0$ and the zero-pattern from Step 1, use an ADMM algorithm to simultaneously re-estimate $theta_0$ and the (normalized) graph Laplacian.
